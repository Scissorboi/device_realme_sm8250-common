...
def AddTrustZoneAssertion(info, input_zip):
  ...
  if m:
    ...
    if len(versions) and '*' not in versions:
      cmd = 'assert(realme_kona.verify_trustzone(' + ','.join(['"%s"' % tz for tz in versions]) + ') == "1" || abort("ERROR: This package requires firmware from an Android 11 based RealmeUI_2 build. Please upgrade firmware and retry!"););'
